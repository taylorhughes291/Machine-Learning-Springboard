...
# Let's mess around with some cutoff-values and see what we can come up with.

PertTest$Prediction <- ifelse(PertTest$PredTest <= 0.675, 0, 1)
TN <- PertTest[PertTest$everwrk == 0 & PertTest$Prediction == 0, ]
FP <- PertTest[PertTest$everwrk == 0 & PertTest$Prediction == 1, ]
FN <- PertTest[PertTest$everwrk == 1 & PertTest$Prediction == 0, ]
TP <- PertTest[PertTest$everwrk == 1 & PertTest$Prediction == 1, ]
CM <- matrix(c(nrow(TN), nrow(FP), nrow(FN), nrow(TP)), nrow = 2, ncol =2)
Sens <- nrow(TP)/(nrow(TP) + nrow(FN))
Sens
Spec <- nrow(TN)/(nrow(TN) + nrow(FP))
Spec
nrow(TN) + nrow(TP)

# After experimenting with a few values, 0.675 seems like the optimal value for getting the most
# correct predictions. Looks like I get an 86.7% success rate for my prediction, whereas I would
# have gotten an 86.55% success rate if you just chose yes every time. Really not too much of an
# improvement but hey, it's something.
...
# Married: 89.2% chance of having ever worked
# Widowed: 80.2% chance of having ever worked
# Separated/Divorced: 93.5% chance of ever having worked
# Never Married: 86.9% chance of having ever worked
